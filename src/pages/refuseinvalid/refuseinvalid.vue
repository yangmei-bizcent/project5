  ...
  <div class="ts-content wrapper-datepicker newcss">
    <main class="main-wrap">
      ...
      <el-table :data="tableData" stripe style="width: 100%" highlight-current-row @current-change="tableCurrentChange">
        ...
        <el-table-column prop="DoctorName" label="送转医生" width="130"></el-table-column>
        <el-table-column prop="LastUpdateTime" label="拒绝时间" width="130"> </el-table-column>
        ...
        <el-table-column prop="RefusalReasonText" label="拒绝原因" width="140"></el-table-column>
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            <span class="check_btn el-icon-ts-serch" @click='checkdetail(scope.row.TransferId,scope.row.AppointmentState,scope.row.PatientName)'>查看详情</span>
            <span class="angin_btn el-icon-ts-cehui" @click='againdetail(scope.row.DischargedPatientId,scope.row.TransferId)'>重新送转</span>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
...
export default {
  ...
  mounted() {
    ...
    // 表格
    axios.get(apiurl.GetPatientRefusedList, {
      params: {
  ...
  methods: {
    ...mapActions([
      'set_waitid', 'set_waitstate', 'set_resend1', 'set_seedpatiendid','set_transferTreatmentBaseId'
    ]),
    datavalue(val) {
      var self = this;
    ...
    tabledata() {
      var self = this;
      // self.sqdata=new Date(self.form.value[0]).getFullYear()+'-'+((new Date(self.form.value[0]).getMonth()+1)>9?(new Date(self.form.value[0]).getMonth()+1):'0'+(new Date(self.form.value[0]).getMonth()+1))+'-'+((new Date(self.form.value[0]).getDate())>9?(new Date(self.form.value[0]).getDate()):'0'+(new Date(self.form.value[0]).getDate()))+'至'+new Date(self.form.value[1]).getFullYear()+'-'+((new Date(self.form.value[1]).getMonth()+1)>9?(new Date(self.form.value[1]).getMonth()+1):'0'+(new Date(self.form.value[1]).getMonth()+1))+'-'+((new Date(self.form.value[1]).getDate())>9?(new Date(self.form.value[1]).getDate()):'0'+(new Date(self.form.value[1]).getDate()));
      // self.sqdata!=='NaN-0NaN-0NaN至NaN-0NaN-0NaN'?self.sqdata:self.sqdata='';
      axios.get(apiurl.GetPatientRefusedList, {
        params: {
          page: self.currentPage,
          rows: self.currentNum,
    ...
    againdetail(id,transferTreatmentBaseId) {
      var self = this;
      self.set_resend1(2);
      self.set_seedpatiendid(id);
      self.set_transferTreatmentBaseId(transferTreatmentBaseId)
      var param = {};
      param.method = "addTab";
      param.title = '新增的病人';
      param.url = linkUrl + 'turnout/addtransfer?token=' + self.token+'&ModelUrl='+this.$route.query.ModelUrl;
      window.parent.postMessage(JSON.stringify(param), '*');
    ...
    exportexl(){
      ...
      self.hrefvalue=apiurl.ExportPatientRefused+'?applyDate='+self.newdatavalue+'&hospitalId='+self.form.Hospitalselect+'&patientName='+self.form.patientName+'&token='+self.token;
