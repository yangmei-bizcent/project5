  ...
  <div class="ts-content wrapper-datepicker newcss nowidth">
    <main class="main-wrap">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="接诊机构">
                <el-select v-model="form.value1" placeholder="请选择" clearable>
      ...
      <el-table :data="tableData" stripe style="width: 100%" highlight-current-row @current-change="tableCurrentChange">
        ...
        <el-table-column prop="LastUpdateTime" label="取消时间" width="140"> </el-table-column>
        ...
        <el-table-column label="操作" width="100">
          <template slot-scope="scope">
            ...
            <span class="angin_btn el-icon-ts-cehui" @click='againdetail(scope.row.DischargedPatientId,scope.row.Id)'>重新送转</span>
          </template>
        </el-table-column>
        <!-- <el-table-column prop="AppointmentStateText" label="进度状态" width="100">
          <template slot-scope="scope">
            ...
            <div v-if='scope.row.AppointmentState==3' class="tag_text3">已取消
              <b></b>
            </div>
          </template>
        </el-table-column> -->
...
export default {
  ...
  mounted() {
    ...
    // 表格
    axios.get(apiurl.GetPatientCanceledList, {
      params: {
  ...
  methods: {
    ...mapActions([
      'set_waitid','set_waitstate','set_seedpatiendid','set_resend1','set_transferTreatmentBaseId'
    ]),
    datavalue(val) {
      var self = this;
    ...
    exportexl(){
      ...
      if(self.form.value1==undefined){
        self.form.value1=''
      }
      if(self.form.patientName==undefined){
        self.form.patientName=''
      }
      self.hrefvalue=apiurl.ExportPatientCanceled+'?applyDate='+self.newdatavalue+'&hospitalId='+self.form.value1+'&patientName='+self.form.patientName+'&token='+self.token;
      console.log(self.newdatavalue,self.form.value1,self.form.patientName,self.token)
