  ...
  <div class="ts-content  wrapper-datepicker newcss nowidth">
    <main class="main-wrap">
      ...
      <el-table :data="tableData" stripe style="width: 100%" highlight-current-row element-loading-text="数据加载中" v-loading="loading" element-loading-spinner="el-icon-loading" element-loading-background="rgba(0, 0, 0, 0.8)" slot="empty" :border='true'>
        <el-table-column label="签约服务收入统计汇总" align='center'>
          <el-table-column prop="HospitalName" label="名称" width="60"> </el-table-column>
          <el-table-column prop="PayServciePackNum" label="有偿签约服务包数量" width="105"> </el-table-column>
          <el-table-column prop="SignDoctorNum" label="参与签约家庭医生数量" width="110"> </el-table-column>
          <el-table-column prop="IncomeOfPayServcie" label="签约服务总收入" width="90"> </el-table-column>
          <el-table-column prop="IncomeOfElderService" label="老年人型服务包总收入" width="115"> </el-table-column>
          <el-table-column prop="IncomeOfDMServcie" label="糖尿病型服务包总收入" width="115"> </el-table-column>
          <el-table-column prop="IncomeOfHTNService" label="高血压型服务包总收入" width="115"> </el-table-column>
          <el-table-column prop="IncomeOfFocalService" label="老高糖型服务包总收入" width="115"> </el-table-column>
        </el-table-column>
      </el-table>
      <div class="block">
...
export default {
  ...
  methods: {
    ...
    coststatisticDate() {
      ...
      axios.get(apiurl.GetPayServicePackFeeStatistics, {
        params:{
          page:this.currentPage,
          rows:this.currentNum,
          prov:this.provCode||'',
    ...
    GetExport() {
      var self = this;
      self.hrefvalue=apiurl.DownloadPayServicePackFeeStatistics+'?token='+this.token+'&prov='+this.provCode+'&city='+this.cityCode+'&area='+this.areaCode+'&year='+this.datavalue1+'&hospitalId='+this.hospitalId+'&doctorId='+this.doctorId;
